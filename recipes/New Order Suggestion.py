...
df_products_not_delivered_exportation = Helpers.getEntityData(context, "df_products_not_delivered_exportation")
df_parametro_fornecedor = Helpers.getEntityData(context, "parametro_fornecedor")
df_daily_portalvendas_components = Helpers.getEntityData(context, "new_daily_portalvendas_components")
df_multi_horizon_pred_component = Helpers.getEntityData(context, "new_multihorizon_components")
df_vendas = Helpers.getEntityData(context, "df_vendas")
df_produtos_sob_demanda = Helpers.getEntityData(context, "produtos_sob_demanda")
df_lista_excecoes_produtos_sem_fornecedores = Helpers.getEntityData(context, "excecoes_produtos_sem_fornecedores")
df_compiled_components = Helpers.getEntityData(context, "new_compiled_components2")
...
df_produto_fornecedor_ativo = Helpers.getEntityData(context, "produto_fornecedor_ativo")
...
if df_produto_fornecedor.shape[0] > 0:
    ...
    if mask_custo_zero.sum() > 0:
        ...
        audit_custo = removed_custo[['PRODUTO']].rename(columns={'PRODUTO': 'Cod_component'})
    ...
    # 2. FILTRO MERGE (FORNECEDOR ATIVO)
    # Simula o merge com indicator para ver quem cai fora
    merge_check = df_produto_fornecedor.merge(
    ...
    if mask_no_match.sum() > 0:
        ...
        audit_merge = removed_merge[['PRODUTO']].rename(columns={'PRODUTO': 'Cod_component'})
        audit_merge['origem'] = 'df_produto_fornecedor'
...
if 'In_Multihorizon_Predictions' not in df_compiled_components.columns:
    ...
    components_with_predictions = set(df_multi_horizon_pred_component["Component"].unique())
    df_compiled_components['In_Multihorizon_Predictions'] = df_compiled_components['Cod_Produto'].isin(components_with_predictions)
    print(f"Componentes com previsões: {df_compiled_components['In_Multihorizon_Predictions'].sum()}/{len(df_compiled_components)}")

# -------------------------------------------------------------------------------- NOTEBOOK-CELL: CODE
def calculate_mean_lead_time(df_historico_pedidos, component_code):
    df_hist = df_historico_pedidos[df_historico_pedidos["PRODUTO"] == component_code].copy()
    if df_hist.shape[0] == 0:
        return np.nan, np.nan
    
    df_hist["Lead_Time"] = (df_hist["ENTREGA_EFET"] - df_hist["DATA_SI"]).dt.days
...
def calculate_safety_stock(
    ...
    df_filtered = df_consumption_true[df_consumption_true["Component"] == component_code].copy()
    ...
    if max_lookback_method in ["orders", "both"]:
        df_filtered = df_filtered.sort_values(by='DATA_PEDIDO', ascending=False)
        df_filtered = df_filtered.head(max_lookback_orders)

    if max_lookback_method in ["months", "both"]:
        df_filtered = df_filtered[df_filtered['DATA_PEDIDO'] >= date - pd.DateOffset(months=max_lookback)]
    
    avg_lt, std_lt = calculate_mean_lead_time(df_historico_pedidos, component_code)
    ...
    if method == 3:
        ...
        safety_stock = Z * math.sqrt((avg_lt * (std_consumption) ** 2 + (avg_consumption * std_lt) ** 2))
    else:
        safety_stock = 0
...
def get_in_transit_orders(date, component, df_historico_pedidos):
    ...
    df_hist = df_hist[(df_hist["ENTREGA_EFET"].isna()) | (df_hist["ENTREGA_EFET"] > date)]
    return df_hist
...
def get_demand_ltrp(base_date, df_forecast, lt_rp, component):
    ...
    df_forecast_low = df_forecast[
        df_forecast['DATA_PEDIDO'] <= base_date + pd.Timedelta(lt_rp, unit="D") + pd.offsets.MonthEnd(0)
    ]
    ...
    forecast_span_low = (
        base_date + pd.Timedelta(lt_rp, unit="D") + pd.offsets.MonthEnd(0) - df_forecast_low['DATA_PEDIDO'].min()
    )
    forecast_span_low_days = forecast_span_low.days

    df_forecast_high = df_forecast[
        df_forecast['DATA_PEDIDO'] <= base_date + pd.Timedelta(lt_rp, unit="D") + pd.offsets.MonthEnd(2)
    ]
    ...
    forecast_span_high = (
        base_date + pd.Timedelta(lt_rp, unit="D") + pd.offsets.MonthEnd(2) - df_forecast_high['DATA_PEDIDO'].min()
    )
    ...
    if forecast_span_high_days == forecast_span_low_days:
        ...
        demand = demand_low + ((demand_high - demand_low) / (forecast_span_high_days - forecast_span_low_days)) * (lt_rp - forecast_span_low_days)
...
def calculate_lead_time_by_supplier(df_historico_pedidos):
    ...
    df_hist["Calculated_Lead_Time"] = (df_hist["ENTREGA_EFET"] - df_hist["DATA_SI"]).dt.days
    df_lt_supplier = df_hist.groupby("COD_FORNE")["Calculated_Lead_Time"].mean().reset_index()
...
def create_df_lt_rp(
    ...
    df_lt_rp = df_lt_rp.merge(df_lt_supplier, how='left', left_on='Cod_Fornecedor', right_on='COD_FORNE')
    
    df_lt_rp = df_lt_rp[["Cod_Produto", "Cod_Fornecedor", "Review_Period", "Lead_Time", "Calculated_Lead_Time"]]
    ...
    df_lt_rp = df_lt_rp.sort_values(by="Supplier")
...
def create_main_dataframe(
    ...
    df_vendas_12meses = df_vendas[df_vendas["DATA"] >= (base_date - pd.DateOffset(months=12))]

    df_main = pd.DataFrame()
    
    # 3. FILTRO: COMPONENTES SEM ESTOQUE ATUAL
    mask_in_stock = df_compiled_components["Cod_Produto"].isin(df_estoque_atual["CODIGO_PI"])
    
    if audit_list is not None and (~mask_in_stock).sum() > 0:
        removed_stock = df_compiled_components[~mask_in_stock].copy()
        audit_stock = removed_stock[['Cod_Produto']].rename(columns={'Cod_Produto': 'Cod_component'})
    ...
    if current_date > max_available_date:
        print(f"Warning: Using latest available date: {actual_inventory_date.date()}")
    ...
    for i, row in df_compiled_components.iterrows():
        ...
        df_vendas_comp = df_vendas_12meses[df_vendas_12meses["B1_COD_PP"] == component]
        ...
        df_inv_comp = df_inv_comp[df_inv_comp["date"] >= (base_date - pd.DateOffset(months=12))]
        ...
        data_row["Unfulfilled 12M"] = unfullfilled_orders_repo + unfullfilled_orders_expo
        ...
        # Pedidos em trânsito
        df_in_transit = get_in_transit_orders(current_date, component, df_order_history)
        transit = df_in_transit["QUANTIDADE"].sum() if "QUANTIDADE" in df_in_transit.columns else 0
        ...
        if component_has_active_supplier:
            ...
            if len(df_forn_ativo_comp) > 0:
                ...
                active_supplier_name = df_forn_ativo_comp["Nome_Fornecedor"].values[0] if not pd.isna(df_forn_ativo_comp["Nome_Fornecedor"].values[0]) else f"Fornecedor_{active_supplier_code}"
                ...
                if df_produto_fornecedor.shape[0] > 0:
                    df_custo = df_produto_fornecedor[
                        ...
                        (df_produto_fornecedor["COD_FORNE"] == active_supplier_code)
                    ]
                    if len(df_custo) > 0:
                ...
                if has_prediction:
                    ...
                    demand_ltrp = int(get_demand_ltrp(base_month, df_forecasts, ltrp, component))
                else:
                    safety_stock = 0
                    demand_ltrp = 0
            ...
            has_prediction = row.get("In_Multihorizon_Predictions", False)
            demand_ltrp = int(get_demand_ltrp(base_month, df_forecasts, ltrp, component)) if has_prediction else 0
    ...
    df_main = df_produtos_select.merge(df_main, left_index=True, right_index=True, how="right")
    ...
    df_main["Total Stock/Sales 12M"] = df_main.apply(
        lambda row: (row["Total Stock"] / row["Sales 12M"] * 100) if row["Sales 12M"] > 0 else 0,
        axis=1
    )
    
    df_main["Order sug/Sales 12M"] = df_main.apply(
        lambda row: (row["Order sug"] / row["Sales 12M"] * 100) if row["Sales 12M"] > 0 else 0,
        axis=1
    )
...
df_inventory_histories = df_inventory_histories.sort_values(["date", "Componente"])
latest_date = df_inventory_histories["date"].max()

# Processar df_code_supplier (com tratamento para df_produto_fornecedor vazio)
if df_produto_fornecedor.shape[0] > 0:
    df_code_supplier = df_produto_fornecedor[["COD_FORNE", "FORNEC_NOM"]].drop_duplicates()
    ...
    df_produto_fornecedor_ativo["Nome_Fornecedor"] = "Fornecedor_" + df_produto_fornecedor_ativo["Cod_Fornecedor"].astype(str)
...
df_components_not_delivered_exportation_grouped = df_components_not_delivered_exportation_grouped.reindex(columns, axis=1)

df_components_not_delivered_exportation_grouped.rename(
    columns={"COD_COMPONENTE": "B1_COD_PP", "DT_ENTREGA": "DATA"}, 
    inplace=True
)

df_vendas_with_exportacao = pd.concat([df_vendas, df_components_not_delivered_exportation_grouped], ignore_index=True)
df_vendas_with_exportacao = (
    df_vendas_with_exportacao
    .groupby(['B1_COD_PP', 'DATA'])[
...
df_historico_pedidos["ENTREGA_EFET"] = pd.to_datetime(df_historico_pedidos["ENTREGA_EFET"])
...
for base_month in base_months:
    ...
    df_main_all_bases = pd.concat([df_main_all_bases, df_main], ignore_index=True)
...
df_main_all_bases = pd.concat([df_main_all_bases, df_main_last], ignore_index=True)
...
# Garantir a ordem das colunas e remover duplicatas (pois o loop roda várias vezes)
df_sem_match = df_sem_match[['Cod_component', 'origem', 'motivo']].drop_duplicates()
Helpers.save_output_dataset(context=context, output_name='df_sem_match_atual_4', data_frame=df_sem_match)
...
print(f"Componentes únicos: {df_main_all_bases['Component'].nunique() if 'Component' in df_main_all_bases.columns else 'N/A'}")
print(f"{'='*60}\n")
Helpers.save_output_dataset(context=context, output_name="lt_rp", data_frame=df_lt_rp)
Helpers.save_output_dataset(context=context, output_name="df_vendas_with_exportacao", data_frame=df_vendas_with_exportacao)
Helpers.save_output_dataset(context=context, output_name="main", data_frame=df_main_all_bases)