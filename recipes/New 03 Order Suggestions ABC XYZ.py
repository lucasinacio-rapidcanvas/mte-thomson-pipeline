...
df_multi_horizon_pred_component = Helpers.getEntityData(context, "new_multihorizon_components_abcxyz")
...
df_compiled_components = Helpers.getEntityData(context, "new_compiled_components2") #new_compiled_components_abcxyz
...
df_historico_pedidos['COD_FORNE'] = df_historico_pedidos['COD_FORNE'].astype(str)

df_parametro_fornecedor = Helpers.getEntityData(context, "parametro_fornecedor")
df_parametro_fornecedor['Cod_Fornecedor'] = df_parametro_fornecedor['Cod_Fornecedor'].astype(str)
...
df_produto_fornecedor_ativo['Cod_Fornecedor'] = df_produto_fornecedor_ativo['Cod_Fornecedor'].astype(str)
...
if df_produto_fornecedor.shape[0] > 0:
    ...
    if mask_custo_zero.sum() > 0:
        ...
        df_audit_cost = df_removed_cost[['PRODUTO']].rename(columns={'PRODUTO': 'Cod_component'})
    ...
    # Simula o merge com indicator para descobrir quem cai fora
    df_merge_check = df_produto_fornecedor.merge(
    ...
    # Captura quem ficaria de fora (quem só existe na esquerda e não na direita)
    mask_no_match = df_merge_check['_merge'] == 'left_only'
    if mask_no_match.sum() > 0:
        df_removed_merge = df_merge_check[mask_no_match].copy()
        df_audit_merge = df_removed_merge[['PRODUTO']].rename(columns={'PRODUTO': 'Cod_component'})
        df_audit_merge['origem'] = 'merge_produto_fornecedor'
        df_audit_merge['motivo'] = 'Sem correspondencia em produto_fornecedor_ativo'
...
def calculate_safety_stock(
    ...
    if max_lookback_method in ["orders", "both"]:
        ...
        df_filtered = df_filtered[
            df_filtered['DATA_PEDIDO'] >= date - pd.DateOffset(months=max_lookback)
        ]
...
def get_demand_ltrp(base_date, df_forecast, lt_rp, component):
    ...
    demand = demand_low + (
        (demand_high - demand_low) / (forecast_span_high_days - forecast_span_low_days)
...
def calculate_lead_time_by_supplier(df_historico_pedidos):
    ...
    df_lt_supplier = df_historico_pedidos.groupby("COD_FORNE")["Calculated_Lead_Time"].mean().reset_index()
...
def create_main_dataframe(
    ...
    if current_date > max_available_date:
        print(f"Warning: Requested date {current_date.date()} is beyond available inventory data.")
    ...
    print(f"Base date: {pd.to_datetime(base_date).date()} | Base month: {pd.to_datetime(base_month).date()}")
    print(f"{'='*80}\n")
    ...
    for idx, (i, row) in enumerate(df_compiled_components.iterrows(), 1):
        ...
        df_inventory_histories_component = df_inventory_histories_component[
            df_inventory_histories_component["date"] >= (base_date - pd.DateOffset(months=12))
        ]
        ...
        if actual_inventory_date in df_inventory_pivot.index:
            ...
            if len(available_dates) > 0:
                ...
                closest_date = min(available_dates, key=lambda x: abs((x - actual_inventory_date).days))
                current_stock = df_inventory_pivot.loc[closest_date, component]
                if idx % 50 == 0 or idx <= 10:
                    print(f"  ⚠️  Data {actual_inventory_date.date()} não encontrada. Usando {closest_date.date()} para {component}")
            else:
                current_stock = 0
                if idx % 50 == 0 or idx <= 10:
                    print(f"  ⚠️  Nenhuma data disponível no histórico para {component}")
        ...
        inspection = df_estoque_atual[df_estoque_atual["CODIGO_PI"] == component]["QTD_TOT_EST_INSP"].values[0]
        reserved = df_estoque_atual[df_estoque_atual["CODIGO_PI"] == component]["QTD_RESE"].values[0]

        df_in_transit_orders = get_in_transit_orders(current_date, component, df_order_history)
        ...
        if component_has_active_supplier:
            df_produto_fornecedor_ativo_component = df_produto_fornecedor_ativo[df_produto_fornecedor_ativo["Cod_Produto"] == component]
            active_supplier_code = df_produto_fornecedor_ativo_component["Cod_Fornecedor"].values[0]
            active_supplier_name = df_produto_fornecedor_ativo_component["Nome_Fornecedor"].values[0]
            active_cod_x = df_produto_fornecedor_ativo_component["Cod_X"].values[0]
            
            df_produto_fornecedor_select = df_produto_fornecedor[df_produto_fornecedor["PRODUTO"] == component][df_produto_fornecedor["COD_FORNE"] == active_supplier_code]
            ...
            if df_produto_fornecedor_select.shape[0] == 0:
                ...
                if idx % 50 == 0 or idx <= 10:
                    print(f"  ⚠️  {component}: Fornecedor ativo não encontrado em produto_fornecedor - PULADO")
                continue
            ...
            if row["In_Multihorizon_Predictions"]:
                if component not in df_sob_demanda["Cod_Produto"].values:
                    ...
                    if df_produtos_component.shape[0] == 0:
                        ...
                        if idx % 50 == 0 or idx <= 10:
                            print(f"  ⚠️  {component}: Produto não encontrado em df_produtos - PULADO")
                        continue
                    ...
                    safety_stock = int(safety_stock_value) if pd.notna(safety_stock_value) else 0
                else:
                    safety_stock = 0
                ...
                demand_ltrp = int(demand_ltrp_value) if pd.notna(demand_ltrp_value) else 0
            else:
                safety_stock = 0
                demand_ltrp = 0
            ...
            if df_produtos_component.shape[0] == 0:
                ...
                if idx % 50 == 0 or idx <= 10:
                    print(f"  ⚠️  {component}: Produto não encontrado para pegar multiplicador - PULADO")
                continue
    ...
    print(f"Sem histórico de estoque: {len(components_without_inventory_history)}")
    print(f"Taxa de sucesso: {100 * len(df_main) / total_components:.1f}% if total_components > 0 else 0%")
    print(f"{'='*80}\n")
    ...
    df_main = df_produtos.merge(df_main, left_index=True, right_index=True, how="right")
...
# -------------------------------------------------------------------------------- NOTEBOOK-CELL: CODE
df_components_not_delivered_exportation = df_products_not_delivered_exportation.merge(df_estru_produtos, how='left', right_on='COD_PRODUTO', left_on='PRODUTO')
df_components_not_delivered_exportation["NAO_ATENDIDOS_EXPO"] = df_components_not_delivered_exportation["G1_QUANT"] * df_components_not_delivered_exportation["QUANT_BO"]

df_components_not_delivered_exportation_grouped = df_components_not_delivered_exportation.groupby(['COD_COMPONENTE','DT_ENTREGA'])['NAO_ATENDIDOS_EXPO'].sum().reset_index()
...
df_components_not_delivered_exportation_grouped.rename(columns={"COD_COMPONENTE": "B1_COD_PP", "DT_ENTREGA": "DATA"}, inplace=True)
...
Helpers.save_output_dataset(context=context, output_name='df_sem_match_atual_5', data_frame=df_sem_match)
...
Helpers.save_output_dataset(context=context, output_name='new_main_abcxyz', data_frame=df_main_all_bases)